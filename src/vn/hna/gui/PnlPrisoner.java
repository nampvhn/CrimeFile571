 ...
 * @author Edison
 */
public class PnlPrisoner extends javax.swing.JPanel {

    Vector vColumnHeader = null;
    Vector vData = null; // Lay tu vAll
    Vector<Prisoners> vAll = null; // Load tu SQLServer
    ...
    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
        ...
        try {
            ...
            Prisoners criminal = new Prisoners();
            criminal.setPrisonerID(Integer.parseInt(vRow.get(0).toString()));
            int id = criminal.getPrisonerID();
    ...
    public Vector loadColumnHeader() {
        ...
        try {
            ...
            vColumnHeader.add("Prison");
            vColumnHeader.add("Time In Prison");
            vColumnHeader.add("Start Date");
            vColumnHeader.add("End Date");
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public void loadTable() {
        ...
        try {
            ...
            vAll = (Vector<Prisoners>) c.viewPrisoners();
            if (vAll.isEmpty()) {
    ...
    public List loadData(List<Prisoners> listAll) {
        ...
        try {
            ...
            vData = new Vector();
            for (Prisoners criminal : listAll) {
                Vector vRow = new Vector();

                vRow.add(criminal.getPrisonerID());
                vRow.add(criminal.getCriminalID().getFullName());
                vRow.add(criminal.getPrisonID().getPrisonName());
                vRow.add(criminal.getTimeInPrison());
                vRow.add(dt.formatedDate(criminal.getStartTime(), "yyyy-MM-dd"));
                vRow.add(dt.formatedDate(criminal.getEndTime(), "yyyy-MM-dd"));
    ...
    public Vector searchByName(Vector<Prisoners> v, String byName) {
        Vector<Prisoners> vFilter = new Vector();
        try {
            for (Prisoners criminal : v) {
                if (criminal.getCriminalID().getFullName().toUpperCase().contains(byName.toUpperCase())) {
